...
/* 1. Devuelve el nombre (solo mayúscula la primera letra), el presupuesto reducido en 250000 seguido por un espacio y la palabra euros, 
...
correspondiente a los equipos y ordenados por el presupuesto en orden descendente. 
Utiliza los siguientes alias de columna en este orden para los presupuestos: "2022-23", "2023-24", "2024-25". */

SELECT INITCAP(nombre), presupuesto -250000 || ' euros' AS "2022-23", presupuesto ||' euros' AS "2023-24", presupuesto * 1.1 || ' euros' AS "2024-25" FROM equipos ORDER BY presupuesto DESC;

/*2. Se quiere el nombre en minúsculas, la fecha de nacimiento y el salario dividido entre 12. Este salario debe redondearse para que tenga solo dos decimales,
...
/*3. Devuelve el nombre (con tres caracteres) seguido de un "-" y el apellido de los fubtolistas en una columna que tenga por alias "NOM-APELLIDO" (emplea la función CONCAT),
y en otra columna el tamaño/número de caracteres del campo apellido de los futbolistas, usando el alias de columna "CARACTERES APELLIDO".
Debe ordenarse descendentemente por ese tamaño de caracteres de apellido.*/

SELECT CONCAT(SUBSTR(nombre,1,3),'-' || apellidos) AS "NON-APELLIDO" FROM futbolistas; 

/*4. Queremos un listado de los nombres de los futbolistas quitando los segundos nombres (nombres compuestos, por ejemplo, borrar LUIS de PEDRO LUIS, quedando solo PEDRO)
y poner como alias de columna "SOLO NOMBRE".*/

SELECT SUBSTR(nombre, 1, INSTR(nombre, ' ')) AS "SOLO NOMBRE" FROM futbolistas; // como se quita el null?
