...
SELECT * FROM cliente WHERE LOWER(nombre_contacto) = 'maria';
...
SELECT REPLACE(linea_direccion1,'C/','CALLE ') FROM cliente WHERE LOWER(ciudad) = 'fuenlabrada';

/* 3. Indica el país, con todas sus letras en mayúsculas, que tiene más clientes. */
SELECT UPPER(pais), (SELECT MAX(COUNT(codigo_cliente)) FROM cliente GROUP by pais) AS "Cantidad" FROM cliente WHERE codigo_cliente = (SELECT MAX(COUNT(codigo_cliente)) FROM cliente GROUP by pais);
...
/* 6. Devuelve en una columna el nombre de contacto, en otra el apellido de contacto y en una tercera el número de palabras
      que hay en el apellido del contacto en la tabla cliente (se debe suponer que solo es posible 1 o 2 apellidos a lo sumo, separados por un espacio siempre). */
...
SELECT nombre_cliente || ': ' || limite_credito || ' euros' FROM cliente WHERE limite_credito BETWEEN 10000 AND 50000;
      
/* 8. Se quiere saber el nombre del empleado y el número de clientes que representa como ventas, ordenados primero por el que tiene más clientes representados
      (de mayor a menor), y luego por el nombre del empleado en orden alfabético (A a Z). */
SELECT e.nombre, COUNT(c.codigo_empleado_rep_ventas) FROM empleado e JOIN cliente c ON e.codigo_empleado = c.codigo_empleado_rep_ventas GROUP BY e.nombre ORDER BY COUNT(c.codigo_empleado_rep_ventas) DESC, e.nombre ASC;
      
/* 9. ¿Cuántos códigos postales de clientes tienen un 8 o un 9 entre sus cifras?. */
SELECT COUNT(codigo_postal) FROM cliente WHERE REGEXP_LIKE(codigo_postal, '[89]');
...
SELECT linea_direccion1, nombre_cliente FROM cliente WHERE linea_direccion2 IS NULL AND LOWER(ciudad) = 'madrid' AND linea_direccion1 LIKE '%4';
