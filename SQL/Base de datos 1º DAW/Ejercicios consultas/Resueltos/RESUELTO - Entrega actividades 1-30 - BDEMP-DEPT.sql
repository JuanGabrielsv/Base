...
/* 5. Calcular el sueldo neto de cada empleado, y la retencion iRPF mensual, sabiendo que la retención es del 19% para el sueldo y de un 10% de la comisión,
      (redondear a 2 decimales). Si el empleado no tiene comisión, se debe mostrar el valor 0. */
SELECT sal, ROUND((sal * -0.19) + sal, 2) AS "Sueldo NETO", ROUND((sal * 1.19) - sal, 2) AS "Retención 19%",
DECODE(comm, null, '0',comm), DECODE(comm, null, '0',(comm * -0.10) + comm) AS "Comisión NETO", DECODE(comm, null, '0',(comm * 1.10) - comm) AS "Retención 10% Comisión" FROM emp;
