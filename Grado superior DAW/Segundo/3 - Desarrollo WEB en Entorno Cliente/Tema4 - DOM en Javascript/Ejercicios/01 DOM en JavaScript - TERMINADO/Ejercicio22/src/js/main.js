...
• Continuamos con el ejercicio anterior para que el producto sea un enlace,cuyo atributo href sea ‘#’.
*/
...
• Los 3 input de tipo text permitirán poder almacenar el nombre, el precio y el código de un nuevo producto al presionar el botón 'Añadir'. 
    */
    ...
    • Continuamos con el ejercicio anterior para que cuando se pase por una fila de la tabla, ponga su fondo de color amarillo.
    */

    const $tBodyTr = $tBody.getElementsByTagName('tr');

    for (let i = 0; i < $tBodyTr.length; i++) {
        $tBodyTr[i].addEventListener('mouseover', () => {
            $tBodyTr[i].style.backgroundColor = 'yellow';
        });
        $tBodyTr[i].addEventListener('mouseout', () => {
            $tBodyTr[i].style.backgroundColor = 'white';
        })
    }
...
• Continuamos con el ejercicio anterior para que el Nombre sea un enlace,cuyo atributo href sea ‘#’. */

    for (let i = 0; i < $tBodyTr.length; i++) {
        const $enlacesNombre = document.createElement('a');
        $enlacesNombre.setAttribute('href', '#');

        if ($tBodyTr[i].firstChild) {
            $enlacesNombre.textContent = $tBodyTr[i].firstChild.textContent;
            $tBodyTr[i].replaceChild($enlacesNombre, $tBodyTr[i].firstChild);
        }
    }
});